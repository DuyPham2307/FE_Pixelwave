...
// Hàm login
export const loginNormal = async (payload: LoginRequest): Promise<LoginResponse> => {
  try {
...
export const getGoogleLoginUrl = async (): Promise<string> => {
  return `${import.meta.env.VITE_API_URL}/api/auth/social-login?provider=google`
};

// Hàm gửi mã xác thực từ Google về backend để lấy token
export const loginGoogle = async (code: string) : Promise<LoginResponse> => {
  try {
    const response = await api.get('/api/auth/social-login/callback', {
...
// Hàm register
export const registerService = async (payload: RegisterRequest): Promise<void> => {
  try {
